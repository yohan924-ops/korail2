...
A Tkinter GUI is included. It needs no extra packages beyond the
library's own dependencies, and runs on Windows, macOS and Linux.
...
- **자동 좌석 대기** — poll for a sold-out train and reserve it when a
  seat opens.
- **내 예약** — list and cancel your reservations.

The password field is masked, is cleared once login succeeds, and is
never written to disk or to the log.

Korail actively blocks automated access (``MACRO ERROR``). The polling
interval is therefore floored at 10 seconds, consecutive errors stop the
loop, and auto-reserve asks for confirmation before it starts. Aggressive
polling risks your account. Use it at your own responsibility.
